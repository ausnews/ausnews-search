...
import org.ausnews.search.data.SimpleQueryBuilder;
...
import static org.ausnews.search.data.SimpleHttpClient.getIntValue;
import static org.ausnews.search.data.SimpleHttpClient.getStringValue;
import static org.ausnews.search.view.SimpleHtmlBuilder.truncate;
...
public class SearchRenderer {
        ...
        if (totalCount(root) > 0 && root.get("root").has("children")) {
            html.div("search-items", (level_1) -> {
                for (JsonNode result : root.get("root").get("children")) {
                    ...
                    Optional<String> title = getStringValue(result, "fields", "headline");
                    Optional<String> body  = getStringValue(result, "fields", "bodytext");
    ...
    static String renderProfileOptions(Map<String, String> properties) {
        ...
        html.option("bm25_freshness", selectedProfile.equalsIgnoreCase("bm25_freshness"));
        return html.build();
    }
