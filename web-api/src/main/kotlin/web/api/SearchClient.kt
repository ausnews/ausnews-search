...
interface SearchClient {
    ...
    public fun topics(
        ...
        @QueryValue select: String = "all(group(group_doc_id) max(15) order(-max(relevance())) each(output(count()) max(3) each(output(summary()))))",
        @QueryValue("presentation.timing") timing: String = "true",
        @QueryValue hits: String = "0",
        @QueryValue firstpubtime: Long = Instant.now().epochSecond - 86400 * 2,
